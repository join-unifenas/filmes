...
> ⚠️ **Não tente rodar `dotnet` no terminal do Ubuntu.** Ele vai responder `command not found` —
...
> ⚠️ **O GitHub não aceita mais a senha da sua conta no terminal.** Se alguma tela pedir
...
> ⚠️ **Daqui em diante, todo comando é executado dentro de `join-api/`**, a menos que o texto
...
> ℹ️ `app.UseHttpsRedirection()` está aqui de propósito. Vamos removê-lo na Parte 7 — guarde
> essa informação.
...
> ⚠️ **Free tier:** o serviço hiberna após ~15 min sem acesso. A primeira requisição depois
...
> ⚠️ **As duas `ENV` precisam estar na segunda etapa.** Variáveis declaradas na etapa `build`
...
> ⚠️ **Este arquivo não vai para o GitHub.** O `.gitignore` do .NET já ignora `.env` — e é
